...
- 当前版本：0.8.1（`package.json` 中的 `version` 与 `publishTime`，以及 `android/app/build.gradle` 中的 `versionCode` / `versionName` 需保持同步）
...
- **漫画图必须显式像素尺寸**：阅读器组件树（GestureDetector + reanimated `Animated.View`）中 RN Image 按 `100%×100%` 布局时既不渲染也不上报 `onLoad`/`onError`，页面永远停在占位图；`ComicImage` / `NativeScrambleImage` 的 `<Image>` 样式必须经 `ComicImageShared.tsx` 的 `useConcreteImageStyle` 换算成具体像素值（外层 Box 仍用 `useFillStyle` 的百分比/高度语义）
